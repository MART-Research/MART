...
public:
	...
	void generateHilbert(int maxDims, int maxBits) {
		#define maxDim (8*sizeof(bitmask_t))
		bitmask_t coord[maxDim], coordPrev[maxDim];
		unsigned i;
		bitmask_t r, r1;
		//assert(point::n >= maxDims);
		int nDims = maxDims;
		int nBits = maxBits;

		if (nDims*nBits > 8*sizeof(r))
		{
			printf("Product of nDims and nBits not exceed %lu.\n", 8*sizeof(r));
			return;
		}

		long n_nodes = 1 << (nDims * nBits);
		for (r = 0; r < n_nodes; ++r)
		{
