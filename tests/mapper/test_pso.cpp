...
// Helper function to run PSO mapping on a given topology.
void test_topology(const std::string& topo_name, Topology* topo, int num_virtual_nodes, int num_clusters) {
    ...
    std::cout << "PSO Mapping Test on " << topo->get_topology_name() 
    ...
    // Set up and run the PSO mapping algorithm.
    PSO pso;
    pso.set_comm_matrix(comm_matrix);
    std::vector<int> mapping = pso.map(vnodes, *topo);
...
int main() {
    // Many-to-One Mapping Tests for PSO. Adjust virtual node counts as needed.
    
    // Test on Mesh2D: 3x3 mesh (9 physical nodes) with 12 virtual nodes.
    {
        Mesh mesh(3, 3);
    ...
    // Test on RingTopology: Ring with 10 physical nodes and 15 virtual nodes.
    {
        Ring ring(10);
