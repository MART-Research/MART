...
// Include Q-Learning mapping algorithm.
...
// Helper function to run Q-Learning mapping on a given topology.
void test_topology(const std::string& topo_name, 
                   Topology* topo, 
                   int num_virtual_nodes, 
                   int num_clusters) {
    // Build the physical topology.
    topo->create_topology();
    std::cout << "Q-Learning Mapping Test on " << topo->get_topology_name() 
    ...
    // Set up Q-Learning mapping algorithm.
    QLearning ql(0.1, 0.0, 0.2); // alpha=0.1, gamma=0.0, epsilon=0.2
    ql.set_comm_matrix(comm_matrix);
    std::vector<int> mapping = ql.map(vnodes, *topo);
...
int main() {
    // Many-to-One Mapping Tests for Q-Learning mapping.
    
    // Test on Mesh: A 3x3 mesh (9 physical nodes) with 12 virtual nodes.
    {
        Mesh mesh(3, 3);
        test_topology("Mesh2D", &mesh, 12, 3);
    }
    
    // Test on Torus_2D: A 3x3 torus (9 physical nodes) with 12 virtual nodes.
    {
        Torus torus2d(3, 3);
        test_topology("Torus2D", &torus2d, 12, 3);
    }
    
    // Test on Torus_3D: A 3x3x2 torus (18 physical nodes) with 20 virtual nodes.
    {
        Torus torus3d(3, 3, 2);
    ...
    // Test on Hypercube: A 4-dimensional hypercube (16 physical nodes) with 20 virtual nodes.
    {
        Hypercube hyper(4);
        test_topology("HypercubeTopology", &hyper, 20, 3);
    }
    
    // Test on Dragonfly: 4 groups with 4 nodes each (16 physical nodes) with 22 virtual nodes.
    // {
    //     Dragonfly dragonfly(4, 4);
    //     test_topology("DragonflyTopology", &dragonfly, 22, 3);
    // }
    
    // Test on Butterfly: 4 levels with 4 nodes per level (16 physical nodes) with 20 virtual nodes.
    {
