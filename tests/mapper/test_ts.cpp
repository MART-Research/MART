...
// Include the Tabu Search mapping algorithm.
...
// Helper function to run TS mapping on a given topology.
void test_topology(const std::string& topo_name, Topology* topo, int num_virtual_nodes, int num_clusters) {
    ...
    std::cout << "TS Mapping Test on " << topo->get_topology_name() 
    ...
    // Set up TS mapping algorithm.
    TS ts(500, 10, 30, 10.0);
    ts.set_comm_matrix(comm_matrix);  // Now TS uses the communication matrix.
    
    // Compute mapping.
    std::vector<int> mapping = ts.map(vnodes, *topo);
...
int main() {
    // Many-to-One Mapping Tests for TS with communication matrix.
    
    // Test on Mesh (Mesh2D): 3x3 mesh (9 physical nodes) with 12 virtual nodes.
    {
        Mesh mesh(3, 3);
    ...
    // Test on Tree (TreeTopology): Binary tree with branching factor 2 and depth 3 (15 physical nodes) with 18 virtual nodes.
    {
        Tree tree(2, 3);
    ...
    // Test on Ring (RingTopology): A ring with 10 physical nodes; map 15 virtual nodes.
    {
        Ring ring(10);
        test_topology("RingTopology", &ring, 15, 3);
    }
    
    // Test on Hypercube (HypercubeTopology): 4-dimensional hypercube (16 physical nodes) with 20 virtual nodes.
    {
        Hypercube hyper(4);
        test_topology("HypercubeTopology", &hyper, 20, 3);
    }
    
    // Test on Dragonfly (DragonflyTopology): 4 groups with 4 nodes each (16 physical nodes) with 22 virtual nodes.
    // {
    //     Dragonfly dragonfly(4, 4);
    //     test_topology("DragonflyTopology", &dragonfly, 22, 3);
    // }
    
    // Test on Butterfly (ButterflyTopology): 4 levels with 4 nodes per level (16 physical nodes) with 20 virtual nodes.
    {
