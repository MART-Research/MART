...
int main()
{
    ...
    ifstream ifs("config_dor");
    if (!ifs)
    {
        cerr << "ERROR: cannot open config_dor\n";
        return 1;
    }
    string line;
    ...
    if (tn == "dragonfly")
    {
        ...
        dragon = make_unique<Dragonfly>(G, Rg, Nr, Gl, vcs, faulty);
    }
    else if (tn == "butterfly")
    {
        ...
        butterfly = make_unique<Butterfly>(k, vcs, faulty);
    }
    else if (tn == "hypercube")
    {
        int dims = stoi(cfg["num_dimensions"]);
        hypercube = make_unique<Hypercube>(dims, vcs, faulty);
    }
    else if (tn == "mesh")
    {
        int r = stoi(cfg["num_rows"]);
        int c = stoi(cfg["num_cols"]);
        mesh = make_unique<Mesh>(r, c, vcs, faulty);
    }
    else if (tn == "ring")
    {
        int n = stoi(cfg["nodes"]);
        ring = make_unique<Ring>(n, vcs, faulty);
    }
    else if (tn == "torus")
    {
    ...
    else if (tn == "tree")
    {
        ...
        tree = make_unique<Tree>(levels, branches, vcs, faulty);
    }
    else
    {
    ...
    LoadBalancedOblivious lbor(topo);
    ...
    for (int i = 0; i < num_tests; ++i)
    {
        ...
        int dst = stoi(trim(arr[1].substr(1)));

        // parse expected_i
        string ek = "expected_" + to_string(i);
        auto parts = split(cfg[ek], ',');
        vector<int> expected;
        for (auto &p : parts)
            expected.push_back(stoi(trim(p)));

        vector<int> path = lbor.route(src, dst, hopcount, vcs, flow);
    ...
    cout << "All DimensionOrderRouter tests passed!\n";
    return 0;
}
